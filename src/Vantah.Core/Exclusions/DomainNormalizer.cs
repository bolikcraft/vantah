...
public static class DomainNormalizer
{
    ...
    // Требование точки — главный фильтр шумовых строк вывода CLI (напр. «No exclusions
    // configured»), поэтому оно не ослабляется: чистые IPv6-литералы без точки (напр.
    // `2001:db8::1`) под предикат не подходят и исключением не признаются — осознанный
    // компромисс, т.к. site-исключения adguardvpn-cli — это домены/IPv4/CIDR.
