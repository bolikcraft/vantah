...
arm64-ассеты появляются начиная с первого релиза после 0.3.4, поэтому
`sha256sums_aarch64` пока стоит в `SKIP` — реальную сумму вписать при обновлении
на этот релиз (`updpkgsums` подставит обе сразу).
